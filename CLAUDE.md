...
1. ✅ **Клавиатура загораживает экран** — добавить `KeyboardAvoidingView` (СДЕЛАНО)
2. **Кнопки телефона пересекаются** — `SafeAreaView` + `paddingBottom` в tabBarStyle
3. **Статус товара не меняется на Android** — проверить SQL миграцию enum в БД
4. **Обязательное фото при регистрации** — шаг загрузки фото коллекции в LoginScreen
5. **Расширить вишлист** — поля: originalName, characterRu, manufacturer, releaseDate
6. **Коллекционеры поблизости** — кнопка в MapScreen, радиус 5/20 км, формула Haversine
7. **Аукцион** — новый тип продажи (большая фича)
8. **Вход через ВКонтакте** — VK OAuth
